...
def panel_text():
    # /start 面板用小状态，保持简洁，不代替每小时完整状态报告。
    try:
        mem = mem_info()
        node = "正常运行" if node_ok() else "异常"
        total_rx = read_int(state_path("traffic_total_rx"))
        total_tx = read_int(state_path("traffic_total_tx"))
        total_gb = bytes_to_gb(total_rx + total_tx)
        return (
            f"宇宙监察委员会VPS管理局\n\n"
            f"[{CFG['server_name']}]\n\n"
            f"节点: {node}\n"
            f"RAM: {mem['mem_pct']}%\n"
            f"告警: {alert_status_text()}\n"
            f"流量: {total_gb:.2f} GB\n\n"
            f"请选择操作："
        )
    except Exception:
        return f"宇宙监察委员会VPS管理局\n\n[{CFG['server_name']}]\n\n请选择操作："


def send_panel():
    send_message(panel_text(), keyboard())
