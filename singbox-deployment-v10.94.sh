...
generate_grpc_node() {
  ...
  # A usable certificate does not require Certbot or any Nginx interruption.
  if [[ -s "$cert_file" && -s "$key_file" ]] && openssl x509 -checkend 0 -noout -in "$cert_file" >/dev/null 2>&1; then
    echo "Using existing valid TLS certificate for ${domain}; Nginx will not be restarted."
  else
    ...
    # HTTP-01 needs port 80 only while a new certificate is being issued.
    if systemctl is-active --quiet nginx 2>/dev/null; then
      systemctl stop nginx
      nginx_was_running=true
    fi
    if ss -H -lnt "sport = :80" 2>/dev/null | grep -q .; then
      [[ "$nginx_was_running" == true ]] && systemctl start nginx || true
    ...
    if [[ "$nginx_was_running" == true ]] && ! systemctl start nginx; then
      die "Certificate issued, but Nginx could not restart. Fix Nginx before continuing."
    fi
  fi
...
# File Browser owns public HTTPS. gRPC is a loopback-only sing-box inbound
# reached through a dedicated Nginx virtual host on the same public TCP/443.
generate_grpc_node() {
  [[ -f "$INFO_FILE" && -f "$CONFIG_FILE" ]] || die "Install the sing-box node first."
  find_singbox || die "sing-box core was not found."
  command -v nginx >/dev/null || die "File Browser Nginx is not installed. Install the File Browser script first."
  [[ -f /etc/nginx/filebrowser-standalone.conf ]] || die "File Browser standalone Nginx configuration was not found."
  ...
  grep -Eq 'listen[[:space:]]+443([[:space:];]|[[:space:]]+ssl)' /etc/nginx/filebrowser-standalone.conf ||
    die "File Browser is not configured for TCP/443. Re-run the matching install.sh after moving Reality off 443."

  local domain current_ip node_name port uuid private_key public_key short_id server_name
  local cert_dir cert_file key_file tmp_config backup_file grpc_name grpc_link grpc_sub_path
  local grpc_local_port=10000 grpc_nginx_conf=/etc/nginx/filebrowser-shared/singbox-grpc.conf nginx_backup=""
  ...
  key_file="${cert_dir}/privkey.pem"
  if [[ ! -s "$cert_file" || ! -s "$key_file" ]] || ! openssl x509 -checkend 0 -noout -in "$cert_file" >/dev/null 2>&1; then
    command -v certbot >/dev/null || die "certbot is required to issue the gRPC TLS certificate."
    if ss -H -lnt "sport = :80" 2>/dev/null | grep -q .; then
  ...
  mkdir -p /etc/nginx/filebrowser-shared "$SUBSCRIPTION_WEB_DIR"
...
EOF
  if ! nginx -t -c /etc/nginx/filebrowser-standalone.conf; then
  ...
  if ! systemctl restart filebrowser-nginx; then
    [[ -n "$nginx_backup" ]] && cp -a "$nginx_backup" "$grpc_nginx_conf" || rm -f "$grpc_nginx_conf"
  ...
  cat >"$tmp_config" <<EOF
{
  ...
  "inbounds": [
    ...
    {
      "type": "vless",
      "tag": "vless-grpc-loopback",
      "listen": "127.0.0.1",
      "listen_port": $grpc_local_port,
      "users": [{ "uuid": "$uuid" }],
      "transport": { "type": "grpc", "service_name": "$GRPC_SERVICE_NAME" }
    }
  ]
...
GRPC_DOMAIN='$domain'
GRPC_PORT='443'
GRPC_LOCAL_PORT='$grpc_local_port'
GRPC_SERVICE_NAME='$GRPC_SERVICE_NAME'
GRPC_LINK='$grpc_link'
GRPC_SUBSCRIPTION_PATH='$grpc_sub_path'
