...
clean_input() {
  # 去掉 Windows/Xshell 粘贴可能带进来的控制字符，避免 config.json 写坏。
...
write_config() {
  ...
  # 用 Python 写 JSON，避免 shell heredoc 把特殊字符/中文/控制字符写坏。
  python3 - <<'PYCONF'
...
cfg = {
    ...
    "service_name": os.environ.get("SERVICE_NAME", "sing-box").strip() or "sing-box",
    "check_port": os.environ.get("CHECK_PORT", "").strip(),
    "ram_warn": 80,
    "ram_critical": 90,
    "swap_warn": 30,
    "swap_critical": 60,
    ...
    "pause_until": 0,
    "init_rx_gb": os.environ.get("INIT_RX_GB", "").strip(),
    "init_tx_gb": os.environ.get("INIT_TX_GB", "").strip(),
}
...
def cpu_percent():
    # 返回整机 CPU 使用率，按 0~100% 显示，不按核心数叠加。
    # 双核机器空闲时显示 0.0% 是正常的，不代表只识别到一核。
    def read_cpu():
...
def write_status_cache(text):
    try:
        p = state_path("status_cache.txt")
...
def read_status_cache(max_age=20):
    try:
        p = state_path("status_cache.txt")
        if not p.exists():
            return None
        if now_ts() - int(p.stat().st_mtime) > max_age:
            return None
...
if [ -n "$port" ]; then
  ss -H -lnt 2>/dev/null | awk '{{print $4}}' | grep -Eq "(:|])${port}$" && listen="1" || listen="0"
fi
printf 'ACTIVE=%s\nPID=%s\nLISTEN=%s\n' "$active" "$pid" "$listen"
"""
    _, out, _ = run(cmd, timeout=2)
    ...
    node_cn = "正常运行" if active == "active" and (not port or listen == "1") else "异常"
    return node_cn, pid, listen


def status_text_quick():
    # 专门给“状态刷新”按钮用：只读 /proc + df + ss，避免慢查询卡按钮。
    update_traffic()
    cpu = cpu_percent()
    mem = mem_info()
    disk = disk_info()
    node_cn, pid, listen = quick_node_state()
    port = CFG.get("check_port") or "未设置"

    if is_paused():
        remain = int(CFG.get("pause_until", 0)) - now_ts()
        alert_status = f"暂停中（还剩 {max(0, remain // 60)} 分钟）"
    else:
        alert_status = "开启"
    ...
    lines = [
        ...
        f"磁盘: {disk['pct']}% ({disk['used_mb'] / 1024:.1f}/{disk['total_mb'] / 1024:.1f}GB)",
        "",
        f"节点: {node_cn}",
        f"端口: {port}",
        "",
    ...
    text = "\n".join(lines)
    write_status_cache(text)
    return text


def status_text_fast():
    # 状态刷新：10秒内读缓存；过期也只走极速状态，不走慢查询。
    cached = read_status_cache(300)
    if cached:
        return cached
    return status_text_quick()


def cache_get(name, max_age=20):
    try:
        p = state_path(f"{name}.cache")
        if not p.exists():
            return None
        if now_ts() - int(p.stat().st_mtime) > max_age:
            return None
...
def cached_text(name, max_age, builder):
    cached = cache_get(name, max_age)
    if cached:
        return cached
    text = builder()
    cache_set(name, text)
...
def traffic_text_fast():
    return cached_text("traffic_text", 300, traffic_text)


def health_text_fast():
    return cached_text("health_text", 300, health_text_quick)


def node_text_fast():
    return cached_text("node_text", 300, node_text)


def logs_text_fast():
    return cached_text("logs_text", 300, logs_text)
...
def health_text():
    return health_text_quick()


def health_text_quick():
    cpu = cpu_percent()
    mem = mem_info()
    disk = disk_info()
    node_cn, pid, listen = quick_node_state()
    ...
    lines = [
        ...
        f"[{CFG['server_name']}]",
        "",
        "✔ 节点正常" if node_cn == "正常运行" else "✘ 节点异常",
        "✔ 端口正常" if (not port or listen == "1") else f"✘ 端口 {port} 未监听",
...
def latency_text():
    ...
    for name, host in targets:
        code, out, _ = run(
            f"ping -c 1 -W 1 {host} 2>/dev/null | awk -F'/' '/rtt|round-trip/{{printf \"%.1f ms\", $5}}'",
            timeout=2
        )
...
def latency_text_fast():
    return cached_text("latency_text", 300, latency_text)
...
def node_text():
    ...
    # 极速节点状态：只查必要项，避免 systemctl show / journal 慢导致按钮卡 10 秒。
    cmd = f"""
...
"""
    ...
    service_cn = status_map.get(active, active or "未知")
    pid_line = pid if pid else "无"

    mem_line = "未知"
    try:
        if rss_kb:
            mem_line = f"{int(rss_kb) / 1024:.1f} MB"
    except Exception:
        pass

    port_line = "未设置"
    if port:
        port_line = f"{port}（{'正常监听' if listen == '1' else '未监听'}）"
...
def logs_text():
    service = CFG.get("service_name", "sing-box")
    code, out, err = run(f"journalctl -u {service} -n 5 --no-pager --no-hostname", timeout=2)
    raw = out or err or "暂无日志"
    if len(raw) > 1200:
        raw = raw[-1200:]
...
def alive_check():
    ...
    # v3.7：每分钟检测，检测到异常就进入自愈流程；重启锁防止风暴。
...
def cache_refresh_fast():
    # 每分钟预刷新：给按钮秒回用。
    # 这里后台慢一点没关系，用户点按钮时只读缓存。
    try:
        s = status_text_quick()
        write_status_cache(s)
    except Exception as e:
        log_event(f"cache status error: {e}")

    try:
        cache_set("traffic_text", traffic_text())
    except Exception as e:
        log_event(f"cache traffic error: {e}")

    try:
        cache_set("node_text", node_text())
    except Exception as e:
        log_event(f"cache node error: {e}")

    try:
        cache_set("health_text", health_text_quick())
    except Exception as e:
        log_event(f"cache health error: {e}")


def cache_refresh_slow():
    # 慢按钮后台刷新：延迟/日志不再等用户点击时现查。
    try:
        cache_set("latency_text", latency_text())
    except Exception as e:
        log_event(f"cache latency error: {e}")

    try:
        cache_set("logs_text", logs_text())
    except Exception as e:
        log_event(f"cache logs error: {e}")


def report():
    text = status_text_quick()
    write_status_cache(text)
    send_message(text, main_keyboard())


def should_skip_callback(chat_id, data):
    # 防重复点击：同一个聊天、同一个按钮，3秒内只处理一次。
    # 避免“第一次慢，第二次点，结果出来两条”。
    key = f"callback_lock_{chat_id}_{data}".replace("/", "_").replace(" ", "_")
    p = state_path(key)
    now = now_ts()
    last = read_int(p, 0)
    if now - last < 2:
...
def handle_callback(callback, already_answered=False):
    data = callback.get("data", "")
    cid = callback.get("id", "")
    msg = callback.get("message", {})
    chat_id = str(msg.get("chat", {}).get("id", ""))

    if should_skip_callback(chat_id, data):
        if not already_answered:
            answer_callback(cid, "处理中，请勿重复点击")
        return

    if not already_answered:
        answer_callback(cid, "已收到，正在处理")

    if data == "status":
        send_message(status_text_fast(), main_keyboard())
    elif data == "health":
        send_message(health_text_fast(), main_keyboard())
    elif data == "traffic":
        send_message(traffic_text_fast(), main_keyboard())
    elif data == "latency":
        send_message(latency_text_fast(), main_keyboard())
    elif data == "node":
        send_message(node_text_fast(), main_keyboard())
    elif data == "logs":
        send_message(logs_text_fast(), main_keyboard())
    elif data == "clean":
...
def dispatch_callback(callback):
    # 先立刻确认按钮，避免 Telegram 客户端一直转圈。
    try:
        answer_callback(callback.get("id", ""), "已收到，正在处理")
    except Exception:
        pass

    def runner():
        try:
            handle_callback(callback, already_answered=True)
        except Exception as e:
            try:
                log_event(f"callback error: {e}")
                msg = callback.get("message", {})
                chat_id = str(msg.get("chat", {}).get("id", ""))
                if chat_id == str(CFG["chat_id"]):
                    send_message(f"⚠️ 按钮处理异常: {e}", main_keyboard())
            except Exception:
                pass
...
def sync_updates():
    # 清掉 Telegram Bot API 里排队的旧消息/旧按钮回调。
    # 多次安装、重复点击、旧脚本崩溃后，Telegram 可能积压 update，
    # 导致新按钮要排队，表现为“卡住，过会儿一下出来两条”。
    token = CFG["bot_token"]
    ...
    try:
        ...
        if updates:
            ...
            write_int(state_path("telegram_offset"), max_id + 1)
        else:
            write_int(state_path("telegram_offset"), 0)
    except Exception:
        pass
...
def bot_poll():
    ...
    params = urllib.parse.urlencode({"timeout": 1, "offset": offset, "limit": 5})
    ...
    for upd in resp.get("result", []):
        ...
        if text in ("/start", "/menu", "菜单"):
            ...
            send_message(logs_text_fast(), main_keyboard())
        elif text in ("/botstatus", "按钮服务"):
            code, out, err = run("systemctl is-active universe-vps-manager-bot; systemctl status universe-vps-manager-bot --no-pager | head -n 12", timeout=10)
            send_message("🤖 按钮服务状态\n\n" + (out or err or "暂无输出"), main_keyboard())
        ...
        elif text in ("/ping", "ping", "测试"):
            send_message("pong", main_keyboard())
        elif text in ("/alert", "/alertstatus", "告警"):
            if is_paused():
                remain = int(CFG.get("pause_until", 0)) - now_ts()
                send_message(f"🟡 告警状态: 暂停中，还剩 {max(0, remain // 60)} 分钟", main_keyboard())
            else:
                send_message("🟢 告警状态: 开启", main_keyboard())
        else:
...
def bot_loop():
    # 常驻 long polling：按钮 / 指令低延迟响应。
    # 监控仍然走 cron，不把监控逻辑塞进常驻进程。
    sync_updates()
    while True:
        try:
            bot_poll()
...
def main():
    if len(sys.argv) < 2:
        print("usage: vps_manager.py alive|resource|report|bot|bot-loop|sync-updates|cache-refresh|cache-slow|menu|clean|status|init-traffic")
        return
...
# 按钮缓存预刷新：每分钟，保证状态/节点/流量/健康尽快返回
* * * * * root python3 $PY_FILE cache-refresh >/dev/null 2>&1

# 慢按钮缓存预刷新：每 5 分钟，避免延迟/日志按钮现查卡顿
*/5 * * * * root python3 $PY_FILE cache-slow >/dev/null 2>&1
...
install_bot_service() {
  echo "[5/6] 创建 Telegram 秒响应服务..."

  # 先停掉旧的按钮服务，避免多个 getUpdates 抢同一个 Bot Token。
  systemctl stop universe-vps-manager-bot.service >/dev/null 2>&1 || true
  pkill -f "/opt/universe-vps-manager/vps_manager.py bot-loop" 2>/dev/null || true
...
EOF
  ...
  if systemctl is-active --quiet universe-vps-manager-bot.service; then
    echo "按钮服务已启动：universe-vps-manager-bot.service"
  else
    echo "警告：按钮服务启动失败，请执行：journalctl -u universe-vps-manager-bot -n 50 --no-pager"
  fi
}
...
finish_install() {
  ...
  python3 "$PY_FILE" cache-refresh >/dev/null 2>&1 || true
  python3 "$PY_FILE" cache-slow >/dev/null 2>&1 || true
  ...
  echo "运行模式：消息内按钮 long polling，按钮数据使用后台缓存，监控仍然使用 cron"
}

main() {
  need_root
  echo "======================================"
  echo "🌌 宇宙监察委员会VPS管理局 v3.7-instant standalone"
  echo "======================================"
  install_deps
  mkdir -p "$APP_DIR"
  clean_old_bots
