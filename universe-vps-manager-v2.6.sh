...
def node_text():
    ...
    _, active, _ = run(f"systemctl is-active {service}", timeout=8)
    _, enabled, _ = run(f"systemctl is-enabled {service} 2>/dev/null || true", timeout=8)
    _, pid, _ = run(f"systemctl show -p MainPID --value {service} 2>/dev/null", timeout=8)
    _, since, _ = run(f"systemctl show -p ActiveEnterTimestamp --value {service} 2>/dev/null", timeout=8)
    ...
    if pid and pid != "0":
        pid_line = pid
        _, rss_kb, _ = run(f"ps -o rss= -p {pid} 2>/dev/null", timeout=8)
        try:
            mem_line = f"{int(rss_kb.strip()) / 1024:.1f} MB"
        except Exception:
            mem_line = "未知"

    port_line = "未设置"
    if port:
        port_line = f"{port}（{'正常监听' if port_listening() else '未监听'}）"
...
def alive_check():
    ...
    # v2.6：每分钟检测，检测到异常就进入自愈流程；重启锁防止风暴。
...
main() {
  need_root
  echo "======================================"
  echo "🌌 宇宙监察委员会VPS管理局 v2.6-instant standalone"
  echo "======================================"
  install_deps
  mkdir -p "$APP_DIR"
  clean_old_bots
