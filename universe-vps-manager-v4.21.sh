    ...
    for item in items:
        if len(item) > 20:
            if pending:
                ...
                if len(pending) == 2:
                    column_width = visual_width("端口: ") + visual_width(pending[0]) + 4
                    second_column_indent = "\u00a0" * ((column_width * 17 + 5) // 10)
                pending = []
            lines.append(f"{second_column_indent}{item}" if second_column_indent else item)
            second_column_indent = ""
            continue
        pending.append(item)
        if len(pending) == 2:
