...
show_link_qr_pair() {
  local ipv4_link="$1" ipv6_link="$2" left right
  if [[ -z "$ipv6_link" ]]; then
  ...
  qrencode -t ANSIUTF8 "$ipv4_link" >"$left"
  qrencode -t ANSIUTF8 "$ipv6_link" >"$right"
  paste -d '    ' "$left" "$right"
}
...
show_link() {
  ...
  install_dependencies
  echo
  echo "节点二维码：IPv4 在左，IPv6 在右。"
  show_link_qr_pair "$ipv4_link" "$ipv6_link"
}
