...
build_vless_link_from_info() {
  local node_name link_host port uuid public_key short_id server_name
...
show_link() {
  ...
  local link
  # Always derive the share link from the structured fields. This avoids a
  # malformed/stale VLESS_LINK value producing a corrupted QR code.
  link="$(build_vless_link_from_info)"
