...
write_subscription_content() {
  ...
  local node_name public_ip port uuid public_key short_id server_name sub_name sub_path url_host
...
EOF
  chmod 644 "$sub_path"
  ensure_subscription_nginx_mapping "$url_host" "$public_ip"

  cat >"$SUBSCRIPTION_INFO_FILE" <<EOF
SUBSCRIPTION_FILE_NAME='$sub_name'
SUBSCRIPTION_PATH='$sub_path'
SUBSCRIPTION_HTTP_URL='http://${url_host}/sub/${sub_name}'
...
generate_subscription_link() {
  ...
  echo "不新增单独订阅服务；若 80 端口被其他程序占用，请手动把 $sub_path 映射到可访问网站目录。"
}
