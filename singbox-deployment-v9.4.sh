...
SUBSCRIPTION_WEB_DIR="/var/www/sub"
SUBSCRIPTION_NGINX_CONF="/etc/nginx/conf.d/singbox-sub.conf"
SUBSCRIPTION_DEFAULT_HTTP_PORT="80"
SUBSCRIPTION_FALLBACK_HTTP_PORTS="2080 8080 8880 10080 18080 28080"
...
choose_subscription_http_port() {
  local old_port candidate
  old_port="$(read_info_value "$SUBSCRIPTION_INFO_FILE" SUBSCRIPTION_HTTP_PORT)"
  if [[ "$old_port" =~ ^[0-9]+$ ]]; then
    printf '%s' "$old_port"
    return
  fi

  for candidate in "$SUBSCRIPTION_DEFAULT_HTTP_PORT" $SUBSCRIPTION_FALLBACK_HTTP_PORTS; do
  ...
  die "未找到可用订阅 HTTP 端口，请手动释放 80/2080/8080/8880/10080 等端口后重试。"
}

make_http_subscription_url() {
  local host="$1"
  local port="$2"
  local file_name="$3"
  if [[ "$port" == "80" ]]; then
    ...
    printf 'http://%s:%s/sub/%s' "$host" "$port" "$file_name"
  fi
}
...
ensure_nginx_available() {
  ...
  if ! command -v apt-get >/dev/null; then
    echo "提示：未检测到 Nginx，且当前系统不支持 apt-get 自动安装。"
    echo "订阅 YAML 已生成，但需要你手动提供一个静态 Web 服务映射 ${SUBSCRIPTION_WEB_DIR}/ 到 /sub/。"
    return 1
  fi

  echo "未检测到 Nginx，正在自动安装轻量静态 Web 服务..."
  apt-get update -y
  DEBIAN_FRONTEND=noninteractive apt-get install -y nginx
  systemctl enable --now nginx >/dev/null 2>&1 || systemctl start nginx >/dev/null 2>&1 || true
  ...
  echo "警告：Nginx 安装失败，请手动配置静态 Web 服务。"
  return 1
}

ensure_subscription_nginx_mapping() {
  local url_host="$1"
  local public_ip="$2"
  local listen_port="$3"
  local server_names="_"

  if ! ensure_nginx_available; then
    return
  fi

  [[ -n "$url_host" && "$url_host" != "_" ]] && server_names+=" $url_host"
  [[ -n "$public_ip" && "$public_ip" != "$url_host" ]] && server_names+=" $public_ip"

  mkdir -p "$(dirname "$SUBSCRIPTION_NGINX_CONF")"
  cat >"$SUBSCRIPTION_NGINX_CONF" <<EOF
server {
    listen ${listen_port};
...
EOF
  ...
  if nginx -t >/dev/null 2>&1; then
    ...
    echo "Nginx 已自动配置 /sub/ 静态订阅目录，监听端口：${listen_port}。"
  else
    rm -f "$SUBSCRIPTION_NGINX_CONF"
    echo "警告：自动写入 Nginx /sub/ 配置后校验失败，已回滚。"
    echo "请手动把 ${SUBSCRIPTION_WEB_DIR}/ 映射到网站的 /sub/ 路径。"
  fi
}

write_subscription_content() {
  [[ -f "$INFO_FILE" ]] || die "没有已保存的节点信息，请先选择 1 安装/重建节点。"
  install_subscription_dependencies

  local node_name public_ip port uuid public_key short_id server_name sub_name sub_path url_host http_port http_url
  node_name="$(read_info_value "$INFO_FILE" NODE_NAME)"
  public_ip="$(read_info_value "$INFO_FILE" PUBLIC_IP)"
  port="$(read_info_value "$INFO_FILE" PORT)"
  uuid="$(read_info_value "$INFO_FILE" UUID)"
  public_key="$(read_info_value "$INFO_FILE" PUBLIC_KEY)"
  short_id="$(read_info_value "$INFO_FILE" SHORT_ID)"
  server_name="$(read_info_value "$INFO_FILE" SERVER_NAME)"

  node_name="${node_name:-sing-box}"
  public_ip="${public_ip:-YOUR_SERVER_IP}"
  server_name="${server_name:-$SERVER_NAME}"
  [[ -n "$port" && -n "$uuid" && -n "$public_key" && -n "$short_id" ]] ||
    die "节点信息文件损坏，请重新安装节点。"

  sub_name="$(read_info_value "$SUBSCRIPTION_INFO_FILE" SUBSCRIPTION_FILE_NAME)"
  if [[ -z "$sub_name" ]]; then
    sub_name="$(openssl rand -hex 4).yaml"
  fi
  [[ "$sub_name" =~ ^[A-Za-z0-9._-]+\.ya?ml$ ]] || die "订阅文件名异常，请删除 $SUBSCRIPTION_INFO_FILE 后重试。"

  mkdir -p "$SUBSCRIPTION_WEB_DIR" "$CONFIG_DIR"
  sub_path="${SUBSCRIPTION_WEB_DIR}/${sub_name}"
  url_host="$(detect_subscription_host "$public_ip")"
  http_port="$(choose_subscription_http_port)"
  http_url="$(make_http_subscription_url "$url_host" "$http_port" "$sub_name")"
...
EOF
  chmod 644 "$sub_path"
  ensure_subscription_nginx_mapping "$url_host" "$public_ip" "$http_port"

  cat >"$SUBSCRIPTION_INFO_FILE" <<EOF
SUBSCRIPTION_FILE_NAME='$sub_name'
SUBSCRIPTION_PATH='$sub_path'
SUBSCRIPTION_HTTP_PORT='$http_port'
SUBSCRIPTION_HTTP_URL='$http_url'
SUBSCRIPTION_HTTPS_URL='https://${url_host}/sub/${sub_name}'
EOF
  chmod 600 "$SUBSCRIPTION_INFO_FILE"
}

refresh_subscription_if_exists() {
...
generate_subscription_link() {
  ...
  local sub_path http_url https_url
  sub_path="$(read_info_value "$SUBSCRIPTION_INFO_FILE" SUBSCRIPTION_PATH)"
  http_url="$(read_info_value "$SUBSCRIPTION_INFO_FILE" SUBSCRIPTION_HTTP_URL)"
  https_url="$(read_info_value "$SUBSCRIPTION_INFO_FILE" SUBSCRIPTION_HTTPS_URL)"
  echo
  echo "Clash/Mihomo 订阅文件已生成：$sub_path"
  echo "HTTP 订阅链接："
  echo "$http_url"
  echo
  echo "HTTPS 参考链接（需要域名证书和 HTTPS 站点已映射 /sub/）："
  echo "$https_url"
  echo
  echo "说明：脚本只生成静态 YAML 文件；如未安装 Nginx，会自动安装并配置 /sub/ 静态访问。"
  echo "不新增单独订阅服务；若 80 被占用，会自动改用备用端口并体现在 HTTP 链接里。"
  echo "如果 HTTP 链接带端口，请确认 VPS 防火墙和云安全组已放行该 TCP 端口。"
}
...
show_status() {
  local service subscription port server subscription_url bbr swap ntp core
  ...
  if [[ -f "$SUBSCRIPTION_INFO_FILE" ]]; then
    subscription="已生成"
    subscription_url="$(sed -n "s/^SUBSCRIPTION_HTTP_URL='\(.*\)'$/\1/p" "$SUBSCRIPTION_INFO_FILE" | head -n1)"
  else
    subscription="未生成"
  fi
  ...
  echo "订阅：$subscription | ${subscription_url:-未配置}"
...
show_link() {
  ...
  local subscription_http_url subscription_https_url
  subscription_http_url="$(read_info_value "$SUBSCRIPTION_INFO_FILE" SUBSCRIPTION_HTTP_URL)"
  subscription_https_url="$(read_info_value "$SUBSCRIPTION_INFO_FILE" SUBSCRIPTION_HTTPS_URL)"
  if [[ -n "$subscription_http_url" ]]; then
    echo
    echo "HTTP 订阅链接："
    echo "$subscription_http_url"
    echo
    echo "HTTPS 订阅链接："
    echo "$subscription_https_url"
  fi
}
