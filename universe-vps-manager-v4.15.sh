...
def display_ports():
    ...
    for node in xray_nodes():
        labels.append(f"{node['public_port']}（{node['protocol']}）")
...
def xhttp_status():
    ...
    return ("正常" if all(state == "正常" for state in states) else next(state for state in states if state != "正常"), ",".join(node["port"] for node in nodes))
