...
check_domain() {
  ...
  grep -Fxq "$ip" <<<"$resolved" ||
    die "$domain 当前解析为：$(tr '\n' ' ' <<<"$resolved")，其中没有本机公网 IPv4 $ip。请确认 DNS 已生效且 Cloudflare 为仅 DNS（灰云）。"
}
...
EOF
  ...
  # 自动证书必须保持开启；仅关闭自动 HTTP 跳转。
  sed -i -E '/^[[:space:]]*auto_https[[:space:]]+(off|disable_certs)([[:space:]]|$)/d' "$CADDYFILE"

  if ! grep -qE '^[[:space:]]*order[[:space:]]+forward_proxy[[:space:]]+before[[:space:]]+reverse_proxy' "$CADDYFILE"; then
    if grep -qE '^[[:space:]]*\{' "$CADDYFILE"; then
      sed -i '0,/^[[:space:]]*{/s//{\n    order forward_proxy before reverse_proxy/' "$CADDYFILE"
    else
      local tmp
      tmp="$(mktemp)"
      cat >"$tmp" <<'EOF'
...
EOF
  ...
  if ! grep -qF "import ${CADDY_SITE_DIR}/*.caddy" "$CADDYFILE"; then
...
write_caddyfile() {
  local domain="$1" port="$2" username="$3" password="$4" site_file route_file site_address fb_file
  site_file="${CADDY_SITE_DIR}/naive-${domain}.caddy"
  route_file="${CADDY_ROUTE_DIR}/${domain}/naive.caddy"
  fb_file="${CADDY_SITE_DIR}/filebrowser-${domain}.caddy"

  ensure_main_caddyfile
  cleanup_managed_naive_config "$domain"
  install -d -m 750 -o root -g "$SERVICE_USER" "$CADDY_ROUTE_DIR/$domain"

  if [[ -f "$fb_file" && "$port" == "443" ]]; then
    # 同域名共用时，File Browser 配置必须主动导入该路由目录。
    grep -qF "${CADDY_ROUTE_DIR}/${domain}" "$fb_file" ||
      die "检测到同域名 File Browser 配置，但它没有导入 ${CADDY_ROUTE_DIR}/${domain}，无法安全合并 NaiveProxy。"
    cat >"$route_file" <<EOF
forward_proxy {
...
EOF
    ...
    if [[ "$port" == "443" ]]; then
      # Naive 的 CONNECT 请求 Host 是目标站点，不是代理域名；:443 必须放在前面。
