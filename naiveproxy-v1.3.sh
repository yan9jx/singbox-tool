...
write_shared_caddyfile() {
  ...
  # 该主配置由网盘脚本和 NaiveProxy 脚本共同维护。
  # forward_proxy 放在 Naive 独立站点的 route 内，因此这里不能再设置全局 order，
  # 否则普通网站的 reverse_proxy 会被 forward_proxy 提前截获。
  cat >"$CADDYFILE" <<EOF
{
    admin off
...
migrate_v12_filebrowser_merge() {
  ...
  # 清理旧版“同域名注入 route”残留；v1.3 只使用独立站点文件。
  ...
  if [[ "$restored" == true ]]; then
    echo "已恢复 v1.2 移走的 File Browser 独立站点配置。"
  fi
  [[ -d "$archive" ]] && echo "旧合并配置已备份到：$archive"
}
...
ensure_distinct_filebrowser_domain() {
  ...
  if [[ -f "${CADDY_SITE_DIR}/filebrowser-${domain}.caddy" ]]; then
    die "NaiveProxy 域名 $domain 已被 File Browser 使用。两个服务必须使用不同域名，但可以解析到同一个 VPS IP。"
  fi
}
...
write_caddyfile() {
  ...
  if [[ "$port" == "443" ]]; then
    # :443 必须排在 Naive 域名前，才能处理 CONNECT 目标为任意网站的代理请求。
...
$site_address {
    ...
    # route 固定 forward_proxy 的处理位置，只作用于 Naive 站点；
    # 其他域名仍由各自独立的 reverse_proxy/file_server 处理。
    route {
        forward_proxy {
...
repair_shared_caddy() {
  ...
  username="$(info_value USERNAME)"
  password="$(info_value PASSWORD)"
  valid_domain "$domain" || die "现有 NaiveProxy 域名无效。"
  validate_port "$port" || die "现有 NaiveProxy 端口无效。"
  [[ -n "$username" && -n "$password" ]] || die "现有节点认证信息不完整。"
  ...
  validate_caddy "$BIN"
  start_service "$port"
  echo "共享 443 配置已修复：NaiveProxy 与 File Browser 使用不同域名、同一 Caddy。"
}
