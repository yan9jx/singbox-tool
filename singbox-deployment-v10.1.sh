...
# Managed by singbox-deployment-v10.1. Static subscription files only.
location ^~ /sub/ {
  ...
  awk -v inc="$include_line" '
    BEGIN { in_server=0; depth=0; inserted=0 }
    {
      ...
      if (!inserted && in_server && depth == 1 && closes > 0) {
        print "";
        print "    # singbox-deployment-v10.1: static subscription path.";
        print inc;
        inserted=1;
      }
      print $0
