...
def status_text_fast():
    # 按钮“状态刷新”优先读 20 秒缓存，避免重复点击时堆积。
    # 缓存没有或过期时才重新采样。
    cached = read_status_cache(20)
    if cached:
        return cached
    text = status_text()
...
def health_text_fast():
    return cached_text("health_text", 20, health_text)


def node_text_fast():
    return cached_text("node_text", 30, node_text)


def logs_text_fast():
    return cached_text("logs_text", 30, logs_text)
...
def latency_text_fast():
    return cached_text("latency_text", 60, latency_text)
...
def node_text():
    ...
    # 合并查询，避免多个 systemctl/ss 分开执行导致按钮慢。
...
listen=""
if [ -n "$port" ]; then
  ss -lnt | awk '{{print $4}}' | grep -Eq "(:|])${port}$" && listen="1" || listen="0"
fi
printf 'ACTIVE=%s\\nENABLED=%s\\nPID=%s\\nSINCE=%s\\nRSS=%s\\nLISTEN=%s\\n' "$active" "$enabled" "$pid" "$since" "$rss" "$listen"
"""
    _, out, _ = run(cmd, timeout=6)
    ...
    enabled_cn = enabled_map.get(enabled, enabled or "未知")

    pid_line = pid if pid and pid != "0" else "无"
    mem_line = "未知"
    try:
        if rss_kb:
...
def logs_text():
    service = CFG.get("service_name", "sing-box")
    code, out, err = run(f"journalctl -u {service} -n 8 --no-pager", timeout=10)
...
def alive_check():
    ...
    # v2.8：每分钟检测，检测到异常就进入自愈流程；重启锁防止风暴。
...
main() {
  need_root
  echo "======================================"
  echo "🌌 宇宙监察委员会VPS管理局 v2.8-instant standalone"
  echo "======================================"
  install_deps
  mkdir -p "$APP_DIR"
  clean_old_bots
