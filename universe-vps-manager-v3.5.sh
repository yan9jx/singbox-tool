...
def alive_check():
    ...
    # v3.5：每分钟检测，检测到异常就进入自愈流程；重启锁防止风暴。
...
def main():
    if len(sys.argv) < 2:
        print("usage: vps_manager.py alive|resource|report|bot|bot-loop|sync-updates|menu|clean|status|init-traffic")
        return
...
finish_install() {
  ...
  echo "运行模式：消息内按钮面板 long polling，监控仍然使用 cron"
}

main() {
  need_root
  echo "======================================"
  echo "🌌 宇宙监察委员会VPS管理局 v3.5-instant standalone"
  echo "======================================"
  install_deps
  mkdir -p "$APP_DIR"
  clean_old_bots
