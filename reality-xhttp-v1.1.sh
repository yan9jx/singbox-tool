#!/usr/bin/env bash
# VLESS + XHTTP + REALITY installer for Debian/Ubuntu
...
generate_reality_keys() {
  ...
  PRIVATE_KEY="$(printf '%s\n' "$output" | sed -nE 's/.*Private key:[[:space:]]*([^[:space:]]+).*/\1/p')"
  PUBLIC_KEY="$(printf '%s\n' "$output" | sed -nE 's/.*Public key:[[:space:]]*([^[:space:]]+).*/\1/p')"
  [[ -n "$PRIVATE_KEY" && -n "$PUBLIC_KEY" ]] || die "无法生成 REALITY 密钥。请确认 Xray-core 版本足够新。"
}
...
write_config() {
  local uuid="$1" port="$2" server_name="$3" private_key="$4" short_id="$5" path="$6"
  mkdir -p "$XRAY_DIR"
  cat >"$XRAY_CONFIG" <<EOF
{
...
install_node() {
  install_deps
  install_xray
  local port uuid server_name short_id path name link
  ...
  read -r -p "REALITY SNI [www.microsoft.com]: " server_name
  ...
  generate_reality_keys
  write_config "$uuid" "$port" "$server_name" "$PRIVATE_KEY" "$short_id" "$path"
  test_config
  write_service
  start_service
