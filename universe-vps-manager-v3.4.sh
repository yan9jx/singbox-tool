...
def main_keyboard():
    # v3.4：普通键盘按钮，不再使用 Inline callback。
    # 好处：不会出现 Inline 按钮长时间转圈；点击后本质是发送一条文字。
    return {
        "keyboard": [
            ["📊 状态刷新", "🧪 健康检查"],
            ["📊 流量统计", "🌐 延迟测试"],
...
def traffic_text_fast():
    return cached_text("traffic_text", 20, traffic_text)


def health_text_fast():
    return cached_text("health_text", 20, health_text_quick)


def node_text_fast():
    return cached_text("node_text", 10, node_text)


def logs_text_fast():
    return cached_text("logs_text", 60, logs_text)
...
def latency_text_fast():
    return cached_text("latency_text", 120, latency_text)
...
def alive_check():
    ...
    # v3.4：每分钟检测，检测到异常就进入自愈流程；重启锁防止风暴。
...
def handle_text_message(text):
    # 普通键盘 / 文本指令处理。慢操作在线程里跑，不阻塞 bot-loop。
    if text in ("/start", "/menu", "菜单"):
        send_message(f"🌌 宇宙监察委员会VPS管理局\n[{CFG['server_name']}]\n\n请选择操作：", main_keyboard())
    elif text in ("/status", "状态", "📊 状态刷新"):
        send_message(status_text_fast(), main_keyboard())
    elif text in ("/health", "健康", "🧪 健康检查"):
        send_message(health_text_fast(), main_keyboard())
    elif text in ("/traffic", "流量", "📊 流量统计"):
        send_message(traffic_text_fast(), main_keyboard())
    elif text in ("/latency", "延迟", "🌐 延迟测试"):
        send_message(latency_text_fast(), main_keyboard())
    elif text in ("/node", "节点", "📦 节点状态"):
        send_message(node_text_fast(), main_keyboard())
    elif text in ("/logs", "日志", "📄 最近日志"):
        send_message(logs_text_fast(), main_keyboard())
    elif text in ("/clean", "清理", "🧹 清理缓存"):
        send_message(f"🧹 正在清理缓存...\n[{CFG['server_name']}]", main_keyboard())
        clean_cache(manual=True)
    elif text in ("/pause10", "暂停", "🟡 暂停告警10分钟"):
        set_pause(10)
        send_message(f"🟡 已暂停告警 10 分钟\n[{CFG['server_name']}]", main_keyboard())
    elif text in ("/resume", "恢复", "🟢 恢复告警"):
        clear_pause()
        send_message(f"🟢 已恢复告警\n[{CFG['server_name']}]", main_keyboard())
    ...
    elif text in ("🔄 重启节点", "/restart-node"):
        send_message(f"🔄 正在手动重启节点...\n[{CFG['server_name']}]", main_keyboard())
        try_restart_node(auto=False)
    elif text in ("🔄 重启 Nginx", "/restart-nginx"):
        send_message(f"🔄 正在重启 Nginx...\n[{CFG['server_name']}]", main_keyboard())
        code, _, _ = run("systemctl list-unit-files nginx.service --no-legend 2>/dev/null | grep -q nginx")
        if code == 0:
            run("systemctl restart nginx", timeout=20)
            ...
            send_message("未检测到 nginx.service", main_keyboard())
    elif text in ("⚠️ 重启 VPS", "/reboot"):
        send_message("⚠️ 确认要重启 VPS，请发送：确认重启VPS", main_keyboard())
    elif text == "确认重启VPS":
        send_message("⚠️ VPS 即将重启。")
        subprocess.Popen("sleep 2; reboot", shell=True)
    else:
...
def dispatch_message(text):
    def runner():
        try:
            ...
            try:
                log_event(f"message error: {e}")
                send_message(f"⚠️ 指令处理异常: {e}", main_keyboard())
            except Exception:
                pass
...
def bot_poll():
    ...
    params = urllib.parse.urlencode({"timeout": 2, "offset": offset})
...
def main():
    if len(sys.argv) < 2:
        print("usage: vps_manager.py alive|resource|report|bot|bot-loop|menu|clean|status|init-traffic")
        return
...
finish_install() {
  ...
  echo "运行模式：按钮面板常驻 long polling，监控仍然使用 cron"
}

main() {
  need_root
  echo "======================================"
  echo "🌌 宇宙监察委员会VPS管理局 v3.4-instant standalone"
  echo "======================================"
  install_deps
  mkdir -p "$APP_DIR"
  clean_old_bots
