...
def display_ports():
    ...
    for port in configured_ports():
        if grpc_public and port == grpc_local:
            labels.append(f"{grpc_public}（{protocols.get(port, 'gRPC')}）")
        else:
            labels.append(f"{port}（{protocols.get(port, '未知协议')}）")
    xhttp_state, _ = xhttp_status()
    if xhttp_state != "未安装":
        labels.append(f"{xhttp_public_port()}（{xhttp_protocol()}）")
...
def xhttp_info():
    info = Path("/etc/xray-xhttp/node-info.env")
    if not info.exists():
        return {}
    values = {}
    for line in info.read_text(errors="ignore").splitlines():
...
def xhttp_public_port():
    link = str(xhttp_info().get("LINK", ""))
    matched = re.search(r"@[^:?#]+:(\d+)", link)
    return matched.group(1) if matched else "443"


def xhttp_protocol():
    config = Path("/etc/xray-xhttp/config.json")
    if not config.exists():
        return "Xray"
    try:
        inbound = json.loads(config.read_text(errors="ignore")).get("inbounds", [{}])[0]
        stream = inbound.get("streamSettings") or {}
        network = str(stream.get("network", "")).strip().lower()
        labels = {
            "xhttp": "XHTTP", "splithttp": "XHTTP", "grpc": "gRPC",
            "ws": "WebSocket", "websocket": "WebSocket", "httpupgrade": "HTTPUpgrade",
            "tcp": "TCP", "raw": "TCP",
        }
        return labels.get(network, network.upper() if network else str(inbound.get("protocol", "Xray")).upper())
    except Exception:
        return "Xray"


def xhttp_status():
    unit_code, _, _ = run("systemctl cat xray-xhttp.service", timeout=3)
    info = xhttp_info()
    if unit_code != 0 or not info:
        return "未安装", ""
    port = str(info.get("LOCAL_PORT", "")).strip()
    active, _, _ = run("systemctl is-active --quiet xray-xhttp", timeout=3)
    if active != 0:
        return "已关闭", port
    if not port.isdigit():
        return "服务异常", port
    ...
    if code != 0 or not any(expected in line and "xray" in line for line in out.splitlines()):
        return "服务异常", port
    return "正常运行", port
